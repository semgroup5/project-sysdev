...
public static void main(String[] args)
{
	...
	public Client()
	{
		...
		try
		{
            ...
            byte[] initPacket = new byte[1];
            initPacket[0] = 'b';

			DatagramSocket videoSocket = new DatagramSocket(50001);
            DatagramPacket packet = new DatagramPacket(initPacket, 1, new InetSocketAddress(pi,50001));
			videoSocket.send(packet);

            while(true)
            {
                byte[] videoPacketBuffer = new byte[510];
                DatagramPacket videoPacket = new DatagramPacket(videoPacketBuffer, 510);
                videoSocket.receive(videoPacket);
                System.out.println(videoPacketBuffer);
            }
			...
			System.err.println("Couldn't get I/O for the connection to: Raspberry Pi, reason :" + e.getMessage());
			System.exit(1);
		}
